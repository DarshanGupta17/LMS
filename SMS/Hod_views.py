...
def cour():
    for i in Course.objects.all():
        print("course ",i.id , i.name)
    for i in Session_Year.objects.all():
        print("session ",i.id , i.session_start , i.session_end)
cour()
def AddWithCsv():
    data = pd.read_csv(r"C:\Users\ASUS\Downloads\Students.csv")
    for index,row in data.iterrows():
        ...
        email = row['email']
        address = row['address']
        course_id = Course.objects.get(id=row['course_id'])
        session_year_id = Session_Year.objects.get(id=row['session_year_id'])
